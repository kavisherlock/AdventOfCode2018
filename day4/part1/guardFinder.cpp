...
int main () {
  ...
  for (pair<string, vector<int>> time : guardSleepTimes) {
    int sleepTime = 0;
    for (int m : time.second) {
      sleepTime += m;
    }
    if (maxSleepTime < sleepTime) {
      maxSleepTime = sleepTime;
      mostSleepyGuard = time.first;
    }
  }
  int sleepiestMinute = -1;
  int maxMinutes = -1;
  for (int m = 0; m < guardSleepTimes[mostSleepyGuard].size(); m += 1) {
    if (maxMinutes < guardSleepTimes[mostSleepyGuard][m]) {
      maxMinutes = guardSleepTimes[mostSleepyGuard][m];
      sleepiestMinute = m;
    }
  }

  cout << mostSleepyGuard << " " << sleepiestMinute << endl;
  cout << stoi(mostSleepyGuard) * sleepiestMinute << endl;
